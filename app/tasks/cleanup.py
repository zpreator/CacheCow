...
def cleanup_old_files():
    ...
    try:
        ...
        if settings and settings.remove_old_files and settings.download_path:
            logger.info(f"Running cleanup: deleting files older than {settings.clean_threshold} days")
            delete_old_files(settings.download_path, settings.clean_threshold)
        else:
            logger.info("Cleanup skipped: not enabled or no download path configured.")
    finally:
        db.close()
